...
def load_vgg():
    ...
    vgg.load_weights('models/vgg19_weights_tf_dim_ordering_tf_kernels.h5')
    vgg.trainable=False
    ...
    return model
content_image=cv2.resize(cv2.imread('download.jfif'),(224,224))
content_image=tf.image.convert_image_dtype(content_image,tf.float32)
style_image=cv2.resize(cv2.imread('style.jfif'),(224,224))
...
def loss_object(style_outputs,content_outputs,style_target,content_target):
    ...
    return content_loss*content_weight+style_weight*style_loss
